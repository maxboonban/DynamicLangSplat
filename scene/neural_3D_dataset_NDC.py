...
class Neural3D_NDC_Dataset(Dataset):
    ...
    def load_meta(self):
        ...
        videos = [video for video in videos if os.path.isdir(video)]
        videos = sorted(videos)
        #assert False, [len(videos), poses_arr.shape[0]]
    ...
    def load_images_path(self,videos,split):
        ...
        N_time = 0
        countss = 300
        assert countss % self.load_every == 0, "Must be dividable"
        for index, video_path in enumerate(videos):
            ...
            for idx, path in enumerate(images_path):
                if this_count >=countss:break
                if this_count % self.load_every == 0:
