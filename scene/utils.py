...
class Camera:
  ...
  def scale_factor_y(self):
    if len(self.focal_length) == 2:
      return self.focal_length[1] * self.pixel_aspect_ratio
    else:
  ...
  def project(self, points: np.ndarray):
    ...
    pixel_x = self.scale_factor_x * x + self.skew * y + self.principal_point_x
    pixel_y = (self.scale_factor_y * y
