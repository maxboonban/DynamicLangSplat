...
class Linear(torch.nn.Linear):
    ...
    def _get_delta_weight(self, input_time=None, frame_id=None):
        ...
        if self.compression == 'vm':
            ...
            #assert False, [weights_t.shape, self.matrix_t.shape, (weights_t @ self.matrix_t).t().shape, self.weight.shape]
            # weights_t: (Capacity x rank)
            # self.matrix_t: (rank x (Dim*Dim))
            # self.weight:  (Dim x Dim)
            delta_w = self.fuse_op((weights_t @ self.matrix_t).t(), self.weight.view(-1, 1)) # F_out*F_in, C
            # delta_w: ((Dim*Dim), 1)
            #assert False, [(weights_t @ self.matrix_t).t().shape, self.weight.view(-1, 1).shape, delta_w.shape]
        elif self.compression == 'loe':
            grid_query = input_time.view(1, -1, 1, 1) # 1, N, 1, 1
    ...
    def forward(self, input: torch.Tensor, input_time=None, frame_id=None) -> torch.Tensor:
        ...
        #assert False, [input.shape, weight.shape, self.bias.shape]
